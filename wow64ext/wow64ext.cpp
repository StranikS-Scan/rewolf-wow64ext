...
extern "C" __declspec(dllexport) DWORD64 __cdecl GetLastErrorFromX64Call()
{
    typedef DWORD(WINAPI* RtlGetLastWin32Error_t)();

    static RtlGetLastWin32Error_t RtlGetLastWin32Error = nullptr;

    if (nullptr == RtlGetLastWin32Error)
    {
        HMODULE ntdll = GetModuleHandleW(L"ntdll.dll");
        if (nullptr == ntdll) return 0;
        RtlGetLastWin32Error = (RtlGetLastWin32Error_t)GetProcAddress(ntdll, "RtlGetLastWin32Error");
    }
...
extern "C" __declspec(dllexport) SIZE_T __cdecl VirtualQueryEx64(HANDLE hProcess, DWORD64 lpAddress, MEMORY_BASIC_INFORMATION64* lpBuffer, SIZE_T dwLength)
{
    ...
    MEMORY_BASIC_INFORMATION64 tempMBI64;
    DWORD tempSize = sizeof(tempMBI64);

    DWORD64 ret = 0;
    DWORD64 status = X64Call(ntqvm, 6, (DWORD64)hProcess, lpAddress, (DWORD64)0, (DWORD64)&tempMBI64, (DWORD64)tempSize, (DWORD64)&ret);
	if (STATUS_SUCCESS != status)
		SetLastErrorFromX64Call(status);

    if ((ret > 0) && (tempSize == dwLength))
        *lpBuffer = tempMBI64;
	return (SIZE_T)ret;
}
